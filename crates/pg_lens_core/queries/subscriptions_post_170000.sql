-- Logical replication subscriptions for the current database (PG 17+).
--
-- Joins pg_stat_subscription_stats to track apply_error_count and sync_error_count.
-- Uses subtwophasestate (PG 17+) instead of the removed subtwophase column.
SELECT
      ...
      s.substream::text AS streaming_mode,
      ...
      err.apply_error_count::int8 AS apply_error_count,
      err.sync_error_count::int8 AS sync_error_count
 FROM pg_subscription s
 JOIN pg_roles r ON r.oid = s.subowner
 LEFT JOIN pg_stat_subscription stat ON stat.subid = s.oid AND stat.relid IS NULL
 LEFT JOIN pg_stat_subscription_stats err ON err.subid = s.oid
 LEFT JOIN (
      SELECT
          sr.srsubid,
